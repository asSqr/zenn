...
　その後，対応する `FilterSet` と `Use Case` を取得し，`Use Case` を実行し，対応する `Response Serializer` に渡して返すと言う流れです．`_create` (`POST`) の場合は，`Use Case` に渡す前に `Request Serializer` によるバリデーションが入ります．`Serializer` はモデルの `save` 等もできますが，今回はバリデーションとデータ形状の明示のみに使うこととします．以下は `Serializer` の定義の例です：
